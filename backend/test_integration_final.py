...
def run_tests():
    ...
    print(f"통과: {db_passed}, 실패: {db_failed}, 오류: {db_errors}")
    ...
    print(f"통과: {redis_passed}, 실패: {redis_failed}, 오류: {redis_errors}")
    ...
    if warnings:
        print(f"⚠️ 발견된 워닝: {len(warnings)}개")
    ...
    print("\n[4/4] 전체 테스트 요약")
    print("-" * 70)
    print(f"데이터베이스 테스트: 통과 {db_passed}, 실패 {db_failed}, 오류 {db_errors}")
    print(f"Redis 테스트: 통과 {redis_passed}, 실패 {redis_failed}, 오류 {redis_errors}")
    print(f"Interface 테스트 워닝: {len(warnings)}개")
